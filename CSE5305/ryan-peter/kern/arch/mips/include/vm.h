...
// With 32-bit addresses and a page size of 4096,
// we can fit the entire virtual address space in 1024 * 1024 entries,
// which fits perfectly in a two-level 1024-ary tree with no wasted memory
// if each entry if 4 bytes and each node is one page
#define PT_FANOUT 1024

struct pt_leaf {
  paddr_t paddr;
};

// An intermediate node for the page table, holding leaves describing
// the mapping of a single virtual page to a physical one
struct pt_node {
        struct pt_leaf children[PT_FANOUT];
};

// The top level of the page table, which can contain up to 1024
// intermediate nodes.
struct page_table {
