...
int scheduleMain(struct process **procArray, int procArraySize, int maxTimesteps) {
    ...
    for(int i=0; i<procArraySize; i++) {
        printf("%s: ArrivalTime %d, RemainingTime %d, Priority %d\n", procArray[i]->name, procArray[i]->arrivalTime, procArray[i]->remainingTime, procArray[i]->priority);
    }
    printf("-------------------------------------\n");
    ...
    while (t < maxTimesteps) {
        ...
        if(RRQueueidx != 0) { //if this queue is not empty..
            rr(RRQueue, &RRQueueidx, t);
        } else if(STCFQueueidx != 0) {
            srtrf(STCFQueue, &STCFQueueidx, t);
        } else if(FIFOQueueidx != 0) {
            fifo(FIFOQueue, &FIFOQueueidx, t);
        } else if(SJFQueueidx != 0) {
